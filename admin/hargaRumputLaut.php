...
global $conn;
$rumputlaut = query("SELECT * FROM data_rumput_laut");
$sumbulan = query("SELECT SUM(x1) as x1 FROM data_rumput_laut");
$sumperayaan = query("SELECT SUM(x2) as x2 FROM data_rumput_laut");
$sumharga = query("SELECT SUM(y) as y FROM data_rumput_laut");
$sumx1_2 = query("SELECT SUM(x1_2) as x1_2 FROM data_rumput_laut");
$sumx2_2 = query("SELECT SUM(x2_2) as x2_2 FROM data_rumput_laut");
$sumy_2 = query("SELECT SUM(y_2) as y_2 FROM data_rumput_laut");
$sumx1y = query("SELECT SUM(x1y) as x1y FROM data_rumput_laut");
$sumx2y = query("SELECT SUM(x2y) as x2y FROM data_rumput_laut");
$sumx1x2 = query("SELECT SUM(x1x2) as x1x2 FROM data_rumput_laut");
$n = query("SELECT COUNT(*) as n FROM data_rumput_laut WHERE x1 != 0;");
...
<div class="modal fade" id="tambah-data-rumput-laut" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Tambah Data rumputlaut</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form onsubmit="submitForm(event)">
          <input type="hidden" name="rumputlaut" value="true" />
          ...
          <a href="#" onclick="loadContent('hargaRumputLaut')">
...
<div class="sticky-top d-flex flex-wrap flex-md-nowrap align-items-center pt-2 pb-2 mb-3 border-bottom ">
  ...
  <a class="nav-link text-primary border-primary border-bottom " onclick="loadContent('hargaRumputLaut')" href="#" id="NavbarTwo">
...
<div class="content" id="contentHalamanHarga">
        ...
        <tr>
          ...
          <td style="width: 6%;" class="text-center">
            <div class="d-flex justify-content-center">
              <a class="nav-link " href="#" data-bs-toggle="modal" data-bs-target="#update-data-rumput-laut" onclick="loadModalContent(<?= $data['id'] ?>)">
  ...
  <div class="d-flex justify-content-center my-5">
    <button type="button" class="btn btn-outline-primary mx-2" data-bs-toggle="modal" data-bs-target="#tambah-data-rumput-laut">Tambah Data</button>
  ...
  <div class="input-group mt-4 mb-3 d-flex justify-content-center">
    ...
    <input <?php if ($mode === "default") {
              echo 'readonly';
            } ?> type="text" class="form-control fs-3" id="a" value="<?php if ($mode === "custom") { echo $perhitungan[0]['a_' . $halaman];
                                                                                                                  } else {echo $a;} ?>">
    <span class="input-group-text">
      <h2>+ (</h2>
    </span>
    <input <?php if ($mode === "default") {
              echo 'readonly';
            } ?> type="text" class="form-control fs-3" id="b1" value="<?php if ($mode === "custom") {echo $perhitungan[0]['b1_' . $halaman];
                                                                                                     } else {echo $b1;} ?>">
    <span class="input-group-text">
      <h2>*</h2>
    </span>
    ...
    <input <?php if ($mode === "default") {
              echo 'readonly';
            } ?> type="text" class="form-control fs-3" id="b2" value="<?php if ($mode === "custom") {echo $perhitungan[0]['b2_' . $halaman];
                                                                                                    } else {echo $b2;} ?>">
    <span class="input-group-text">
      <h2>*</h2>
    </span>
...
<div class="modal fade" id="update-data-rumput-laut" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update Data Rumput Laut</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
